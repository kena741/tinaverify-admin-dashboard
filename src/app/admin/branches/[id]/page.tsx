...
export default function BranchDetailPage({
	...
	const { tables: branchTables, loading: tablesLoading } = useAppSelector(
		(state: any) => state.tables,
	);
	const { staff: branchStaff, loading: staffLoading } = useAppSelector(
	...
	const restaurantName = useMemo(() => {
		...
		const restaurant = restaurants.find(
			(r: any) => r.id === selectedBranch.restaurant_id,
		);
	...
	const branch = {
		...
		telebirrStatus: selectedBranch.telebirr_merchant_id
			? "connected"
			: "not_connected",
		createdAt: selectedBranch.created_at
	...
	const recentTransactions = [
		...
		},
		{
			...
			telebirrId: "TB-123457",
		},
		{
			id: "TXN-003",
			table: 7,
			waiter: "Meron Tadesse",
			amount: 320,
			status: "success",
			timestamp: "2024-01-15 14:20",
			telebirrId: "TB-123458",
		},
		{
			id: "TXN-004",
			table: 1,
	...
	const analytics = {
		...
		monthlyGrowth: 12.5,
		occupiedTables: branchTables.filter((t: any) => t.status === "ASSIGNED")
			.length,
		availableTables: branchTables.filter((t: any) => t.status === "FREE")
			.length,
	};
					...
					<div className="flex items-center space-x-4">
						...
						<div>
							...
							<div className="flex items-center space-x-4 mt-1">
								<Link
									href={`/admin/restaurants/${branch.restaurant_id}`}
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="mt-2 text-2xl font-bold text-gray-900">
								ETB {analytics.totalRevenue.toLocaleString()}
							</p>
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="mt-1 text-xs text-gray-500">
								Avg: ETB {analytics.avgTransactionValue}
							</p>
						</div>
				...
				<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="mt-1 text-xs text-gray-500">
								{analytics.availableTables} available
							</p>
						</div>
									...
									<div className="space-y-4">
										...
										<div>
											...
											<div className="w-full bg-gray-200 rounded-full h-2">
												<div
													...
													style={{
														width: `${branchTables.length > 0 ? (analytics.occupiedTables / branchTables.length) * 100 : 0}%`,
													}}
												/>
											</div>
										</div>
										...
										<div>
											<div className="flex items-center justify-between mb-2">
												...
												<p className="text-base font-semibold text-gray-900">
													ETB {analytics.avgTransactionValue}
												</p>
											</div>
										</div>
									</div>
										...
										>
											...
											>
												<div
													...
													style={{
														height: `${(item.revenue / maxHourlyRevenue) * 100}%`,
													}}
									...
									{paymentMethods.map((method, index) => (
										<div key={index}>
											...
											<p className="text-xs text-gray-500 mt-1">
												ETB {method.revenue.toLocaleString()}
											</p>
										</div>
									))}
								</div>
					...
					<div className="relative bg-white rounded-xl shadow-xl max-w-md w-full p-6">
						...
						<form
							onSubmit={async (e) => {
								...
								if (
									staffFormData.role === "WAITER" &&
									!staffFormData.email &&
									!staffFormData.phone
								) {
									setStaffFormError(
										"Email or phone number is required for waiter accounts",
									);
									return;
								}
								...
								try {
									...
									const staffResult = await dispatch(
										createStaff({
						...
						>
							...
							<div>
								...
								/>
								<p className="mt-1 text-xs text-gray-500">
									Required: Set a password for staff login (minimum 6
									characters)
								</p>
							</div>
							<div>
								...
								<p className="mt-1 text-xs text-gray-500">
									Staff will be assigned to this branch
								</p>
							</div>
							<div className="flex items-center space-x-3 pt-4">
								<button
