...
export const fetchStaff = createAsyncThunk(
  ...
  async (filters?: { branchId?: string; restaurantId?: string }) => {
    if (filters?.branchId) {
      ...
      return (data?.map((item: any) => item.staff).filter(Boolean) || []) as Staff[];
...
export const createStaff = createAsyncThunk(
  'staff/create',
  async (data: { name: string; phone: string; role: 'WAITER' | 'ADMIN'; email?: string; password: string; restaurant_id?: string; branch_id?: string }, { rejectWithValue }) => {
    try {
      let authUserId: string | null = null;

      // Create Supabase Auth account for both WAITER and ADMIN
      // Use email if provided, otherwise create email from phone
      const phoneEmail = data.phone.replace(/[\s\+\-\(\)]/g, '') + '@staff.local';
      ...
      if (authError) {
        ...
        if (authError.message) {
          ...
          if (
            ...
            errorMessage = 'This email/phone is already registered. Please use a different one.';
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.warn('Could not update phone in auth.users table:', errorData.error);
          // Continue anyway - phone is still in user_metadata
        }
      } catch (updateError: any) {
        // If update fails, continue - phone is still in user_metadata
        console.warn('Could not update phone in auth.users table:', updateError.message);
      }
      ...
      if (data.role === 'WAITER') {
        ...
        if (staffError) {
          ...
          try {
            ...
            console.warn('Could not rollback auth user creation. Manual cleanup may be required:', authUserId);
          }
          ...
          if (staffError.message) {
            ...
            if (message.includes('duplicate') || message.includes('unique') || message.includes('already exists')) {
              errorMessage = 'A staff member with this information already exists. Please check the details.';
            } else {
      ...
      if (data.role === 'ADMIN') {
        ...
        if (adminError) {
          ...
          if (adminError.message) {
            ...
            if (message.includes('duplicate') || message.includes('unique') || message.includes('already exists')) {
              errorMessage = 'An admin with this email/user ID already exists. Please use a different email.';
            } else {
    ...
    builder
      ...
      .addCase(removeStaffFromBranch.fulfilled, (state, action) => {
        state.staffBranches = state.staffBranches.filter(
          (sb) => !(sb.staff_id === action.payload.staffId && sb.branch_id === action.payload.branchId)
        );
      })
